...
- **Admin screens come from the shared `blueworx-admin-design` system**, not from
  ...
  notice or form. Also copy its `styles.css` to `assets/blueworx-admin-design.css`, its
  `design-system.php` to `assets/blueworx-admin-design.php` — beside the stylesheet, since
  it works out its own asset URL from where it sits — its `fonts/` to `assets/fonts/` —
  those exact paths, since the stylesheet loads the webfonts from beside itself — and its
  `assets/icons/lucide-icons.js` to `assets/blueworx-admin-icons.js`. Require the registrar
  and call `blueworx_admin_design_enqueue()` on the plugin's admin pages, and
  `blueworx_admin_design_enqueue_icons()` too on any screen rendered as PHP rather than
  React, or every `data-lucide` element on it renders empty. Don't enqueue the stylesheet
  yourself: on a site with two BlueWorx plugins, whichever enqueued first used to decide
  how every plugin's screens looked, and the registrar loads the newest copy present on
  the site instead; nothing is loaded from a shared package at runtime.
